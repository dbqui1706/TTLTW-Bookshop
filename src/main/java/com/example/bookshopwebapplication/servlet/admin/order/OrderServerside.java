...
@WebServlet(urlPatterns = {"/admin/orders", "/admin/get-orders"})
public class OrderServerside extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        if (request.getRequestURI().contains("/admin/orders")) {
            request.getRequestDispatcher("/WEB-INF/views/admin/order/orderManager-exp.jsp").forward(request, response);
            return;
        }
        ...
        // Tổng số bản ghi
        int totalRecords = orderService.count();

        // Lọc dữ liệu nếu có tìm kiếm
        List<OrderDto> orders = orderService.getOrderPartServerSide(
                length, start,
